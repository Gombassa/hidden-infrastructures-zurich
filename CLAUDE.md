...
Phase 2 complete on `main` branch. Deployed to Cloud Run. Phase 3 (instrument architecture rebuild, replacing `audio-layers.js` — see `docs/Implementation_Plan.md`) is **underway**: the interface contract is resolved (Option A, class-per-instrument, ratified in Step 1), and electricity, water, tram, and sewage (Steps 2–5) are fully rebuilt as `src/instruments/*.js` classes with paired HTML control surfaces under `instruments/` (plus `docs/instrument-reference.html`, a project-wide reference/status doc) — 23 of a 24-item behaviour inventory now built (Water Flow is a new continuous-bed behaviour not in the original 23). Only telecom (Step 6) remains before Step 7 (Fernwärme) and Step 8 (reintegration into `index.html`'s live GPS/audio path, which no instrument is wired into yet). The list below still describes the current *production* (Phase 2) implementation, which the Phase 3 work will eventually replace — do not describe it in past tense as already decomposed into instruments; `audio-layers.js` is still what ships.
...
- Phase 3: interface contract resolved (Option A, ratified Step 1); electricity, water, tram, and sewage (Steps 2–5) fully rebuilt as real instrument classes — 23/24 behaviours built, only telecom's node-entry chirp still unbuilt (see `docs/instrument-reference.html`). The crossing/alongside consolidation (`crossing-family.html`'s finding that 8 of the 24 items share one detection path) held for electricity, water, and sewage's crossing half, but sewage's alongside loop turned out not to fit it — see `docs/Implementation_Plan.md` Step 5 and Decision Point 4 for the revised finding; whether telecom's alongside matches electricity/water's pattern or sewage's is still open, to check in Step 6. Remaining work: Step 6 (telecom, second pool-paradigm checkpoint), Step 7 (Fernwärme), then integrate and retire `audio-layers.js` once feature parity is field-confirmed (Step 8)
