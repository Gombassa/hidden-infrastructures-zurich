...
**Step 2 extends the same contract with a second shared-extraction pattern.** Where `PoolAllocator` extracted claim/steal/margin/refuse bookkeeping once real duplication had already occurred (two prototypes independently reinventing it), `LineCrossingVoice` (`src/instruments/line-crossing-voice.js`) extracts the crossing/alongside detection-and-voicing shape once a real *consolidation opportunity* was confirmed (`crossing-family.html`'s finding, Decision Point 4, above) rather than assumed. Both are instances of the same Option A discipline: instantiate a class per behaviour-family, let a preset or allocator config carry what varies, don't split further than the evidence supports.

**Step 4 adds a third shared-extraction module, `src/instruments/tram-spatial.js`** — the HRTF coordinate transform (`feederToXYZ`) both `feeder-crackle.js` and `tram-hiss-voice.js` need, ported verbatim from production's `_feederToXYZ` (`audio-layers.js`) but as a pure function taking listener state as parameters rather than production's module-closure `_lastListenerLat/Lng/Heading`. HRTF `PannerNode` construction itself stayed un-shared (inline per instrument) — too small and lifecycle-divergent (one-shot burst-group vs. persistent per-voice) to be worth a helper. `tram-spatial.js`'s docblock also carries forward, unfixed, a pre-existing production quirk worth recording here: `index.html`'s `deviceorientation` handler independently rotates `ctx.listener.forward{X,Y,Z}` from the same device heading that `feederToXYZ` also bakes into panner position — a real double-rotation of heading, present in production today, not introduced or corrected by this port (`index.html` stays untouched).

**Step 4 also confirmed the Option A contract generalises to a third, materially different pool shape.** Tram's hiss pool (`tram-hiss-pool.js` + `tram-hiss-voice.js`) is a deliberate departure from production (see the Instrument Inventory table and `docs/Implementation_Plan.md` Step 4 for the full reasoning) — a persistent per-feeder-identity redesign, not a parity port — yet it reuses `PoolAllocator` from Step 1 completely unchanged, with the same steal-furthest+margin policy as electricity. This is arguably a harder test of the allocator's generality than electricity was: tram's voices carry hashed timbre synthesis and per-voice HRTF positioning that the allocator itself never needs to know about, confirming the claim/release bookkeeping really is decoupled from what a "voice" does.

**Step 6 closed the pool-paradigm risk, and found the escape hatch this section already anticipated.** Telecom's burst pool (`telecom-burst-pool.js`) doesn't use `PoolAllocator` at all — its 4 slots are built once at construction and run continuously, gated collectively by aggregate proximity stats (nearest cable distance, cable count within radius), with no per-feature claim/release/steal at any point. This is the always-on collective-gating shape flagged above as most likely to need a contract change — it needed one, but a narrow one: the `Instrument` base class lifecycle (`update()`/`destroy()`) holds for `TelecomBurstPool` exactly as it does for every other instrument, so the contract itself (the actual Step 1 decision) is unaffected. Only the *allocator*, a Step 1 sub-component built for claim/release-by-id, doesn't generalise to a pool with no per-feature identity to claim — which was always a narrower claim than "the pool paradigm contract." Both Step 1-anticipated pool checkpoints (Step 4, Step 6) are now closed with no contract changes needed.
...
|---|---|---|---|
| `lk-sewage.geojson` | 14,128 | pipe: 14,128 | 6.14 MB (6,137,545 B) |
| `lk-electricity.geojson` | 28,152 | cable: 20,217, node: 7,935 | 12.18 MB (12,182,580 B) |
| `lk-water.geojson` | 20,392 | pipe: 14,257, fitting: 6,135 | 7.95 MB (7,950,718 B) |
| `lk-tram-lk.geojson` | 13,016 | trasse: 8,285, node: 2,674, area: 2,057 | 6.35 MB (6,347,934 B) |
| `lk-telecom.geojson` | 33,542 | cable: 25,822, node: 5,179, area: 2,541 | 15.77 MB (15,772,665 B) |
| `lk-fernwaerme.geojson` | 593 | pipe: 593 | 306.7 KB (306,684 B) |

**114 GeoShop orders processed** (55297–57127) · **109,823 total features** across 6 files · **48.70 MB (48,698,126 B)** served.
